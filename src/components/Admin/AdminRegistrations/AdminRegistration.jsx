...
function AdminRegistration() {
  ...
  useEffect(() => {
    const getRegistrations = async () => {
      const res = await axios.get('https://localhost:7249/api/Registrations')
  ...
  const deleteRegistration = async (id) => {
    try {
      ...
      if (confirm) {
        const res = await axios.delete(`https://localhost:7249/api/Registrations/${id}`)
        console.log(res)
      ...
      {registrations && registrations.map((item, index) => (
        <div key={item.reg_Id} className="registration">
          <div className='registration-card-header'>
            ...
            <button onClick={() => deleteRegistration(item.reg_Id)}>Delete</button>
          ...
          <div className='registration-details-box'>
            ...
            <h1>Phone number: {item.phn_No}</h1>
          </div>
        </div>
      ))}
    </div>
