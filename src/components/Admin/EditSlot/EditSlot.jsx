...
function EditSlot() {
  ...
  useEffect(()=>{
    const getSlots = async () => {
      const res = await axios.get("https://localhost:7249/api/Slots")
  ...
  const handleAddSlot = async (e) => {
    ...
    try {
      ...
      await axios.post("https://localhost:7249/api/Slots" ,
        {time : time}
      )
  ...
  const handleDelete = async (id) => {
      ...
      if(confirm) {
        try {
          await axios.delete(`https://localhost:7249/api/Slots/${id}`)
