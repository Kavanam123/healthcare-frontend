...
function EditBooking() {
  ...
  useEffect(() => {
    const getBookings = async () => {
      const res = await axios.get('https://localhost:7249/api/Bookings')
      setbooking(res.data)
    }
    getBookings()
  }, [load])
...
const confirmBooking = async (item) => {
  try {
    ...
    if (crfm) {
      ...
      const res = await axios.put(`https://localhost:7249/api/Bookings/${item.booking_Id}`, item)
      console.log(res)
      ...
      {booking && booking.toReversed().map((item, index) => (
        <div key={item.booking_Id} className="booking">
          ...
          <h1>User name: {item.registration.first_Name}</h1>
            ...
            <h1>Phone number: {item.registration.phn_No}</h1>
          </div>
